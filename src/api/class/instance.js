...
class WhatsAppInstance {
    ...
    constructor(key, allowWebhook = false, webhook = null) {
        ...
        if(webhook !== null) {
            let webhook_data = { webhook: webhook };
            let data = JSON.stringify(webhook_data);
            fs.writeFile ("./src/api/webhook/" + key + ".json", data, function(err) {
                if (err) throw err;
    ...
    setHandler() {
        ...
        sock?.ev.on('contacts.set', async ({ contacts }) => {
            const recivedContacts = contacts.map((contact) => {
                return {
                    ...contact,
                }
            })
            this.instance.contacts.push(...recivedContacts)
            
            await this.updateDb(this.instance.contacts)
        })
    ...
    async sendUrlMediaFile(to, url, type, mimeType, caption = '', fileName = 'File') {
        await this.verifyId(this.getWhatsAppId(to))
    ...
    async sendButtonMessage(to, data) {
        ...
        const result = await this.instance.sock?.sendMessage(
            this.getWhatsAppId(to),buttonMessage)
        return result
    }
    
    async sendButton2Message(to, data) {
        await this.verifyId(this.getWhatsAppId(to))
        const buttonMessage = {
            text: data.text,
            footer: data.footerText,
