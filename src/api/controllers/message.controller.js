...
exports.Button2 = async (req, res) => {
    // console.log(res.body)
    const data = await WhatsAppInstances[req.query.key].sendButton2Message(
        req.body.id,
        req.body.btndata
    )
    return res.status(201).json({ error: false, data: data })
...
exports.Poll = async (req, res) => {
    const data = await WhatsAppInstances[req.query.key].sendPoll(
        req.body.id,
        req.body.poll
    )
    return res.status(201).json({ error: false, data: data })
